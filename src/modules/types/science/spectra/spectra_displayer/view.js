        ...
        $.extend(true, View.prototype, Default, {
            init() {
                ...
                this.highlightOptions = Object.assign(
                    ...
                    Util.evalOptions(
                        this.module.getConfiguration('highlightOptions')
                    )
                );
                    ...
                    if (graphurl) {
                        $.getJSON(graphurl, {}, data => {
                            data.options.onMouseMoveData = (e, val) => {
                                this.module.controller.sendAction(
                                    'mousetrack',
                                    val
                                );
                            };
                        ...
                        const useMouseTracking = cfgCheckbox(
                            'mouseTracking',
                            'track'
                        );
                        if (useMouseTracking) {
                        ...
                        const selectScatterPlugin = cfgCheckbox(
                            'selectScatter',
                            'yes'
                        );
                        ...
                        if (cfg('xaxismodification') == 'timestamptotime') {
                            ...
                            cfg('xaxismodification') == 'valtotime:min.sec'
                        ) {
                        ...
                        if (selectScatterPlugin) {
                            ...
                            plugin.on('selectionEnd', selectedIndices => {
                                ...
                                if (info) {
                                    result = selectedIndices.map(
                                        index => info[index]
                                    );
                                }
                                this.module.controller.onScatterSelection(
                                    result
                                );
                            });
                        }
                ...
                prom
                    .then(graph => {
                        ...
                        graph.on('shapeResized', shape => {
                            this.module.model.dataTriggerChange(
                                shape.getData()
                            );
                        });
            ...
            getYAxis(index) {
                ...
                for (var i = this.numberOfYAxes; i <= index; i++) {
                    ...
                    if (i === 0) {
                        ...
                        yAxis
                            .on('zoom', yZoomHandler)
                            .on('zoomOutFull', yZoomHandler);
                    } else {
                        yAxis
                ...
                if (plotinfos) {
                    for (var i = 0, l = plotinfos.length; i < l; i++) {
                        if (varname == plotinfos[i].variable) {
                            ...
                            options.strokeWidth = parseInt(
                                plotinfos[i].strokewidth
                            );
                ...
                if (plotinfos) {
                    ...
                    for (var i = 0, l = plotinfos.length; i < l; i++) {
                        if (varname == plotinfos[i].variable) {
                            ...
                            const axisIdx =
                                (plotinfos[i].axis
                                    ? Number(plotinfos[i].axis)
                                    : 0) - minAxis;
                            var axis = this.getYAxis(axisIdx);
                            const startSpan =
                                axisIdx * stackVerticalSpacing || 0;
                            const endSpan =
                                1 -
                                stackVerticalSpacing * (nbAxes - 1 - axisIdx);
                            ...
                            if (
                                ...
                                var other = this.getYAxis(
                                    Number(plotinfos[i].adaptTo)
                                );
                            ...
                            if (
                                plotinfos[i].tracking &&
                                plotinfos[i].tracking[0] === 'yes'
                            ) {
            ...
            update: {
                    ...
                    for (let i = 0; i < data.length; i++) {
                        ...
                        if (i === 0 && moduleValue.axis) {
                            if (moduleValue.axis[aData.xAxis]) {
                                this.xAxis.setLabel(
                                    moduleValue.axis[aData.xAxis].label
                                );
                            }
                            if (moduleValue.axis[aData.yAxis]) {
                                this.yAxis.setLabel(
                                    moduleValue.axis[aData.yAxis].label
                                );
                            }
                        }
                        ...
                        let serieOptions = this.getSerieOptions(
                            varname,
                            aData._highlight,
                            [valFinalX, valFinalY]
                        );
                        ...
                        if (String(aData.type) === 'scatter') {
                            ...
                            let keys = new Set(
                                Object.keys(defaultStyles).concat(
                            ...
                            if (
                                ...
                                var plugin = this.graph.getPlugin(
                                    'selectScatter'
                                );
                ...
                xArray(moduleValue, varname) {
                    ...
                    var maxX = this.module.getConfiguration(
                        'maxX',
                        val.length - 1
                    );
                    ...
                    let serieOptions = this.getSerieOptions(varname, null, [
                        null,
                        [val]
                    ]);
                    var serie = this.graph.newSerie(
                    ...
                    function displaySpectra(spectra) {
                        ...
                        if (spectra.contourLines) {
                            ...
                            for (var i = 0, l = spectra.length; i < l; i++) {
                                var data =
                                    spectra[i].data[spectra[i].data.length - 1];
                                ...
                                let serieOptions = that.getSerieOptions(
                                    varname,
                                    null,
                                    data
                                );
                ...
                series_xy1d(data, varname) {
                    ...
                    require(['src/util/color'], Color => {
                        ...
                        for (; i < l; i++) {
                            ...
                            serie.setLineWidth(
                                data[i].lineWidth || opts.strokeWidth || 1
                            );
                            serie.setLineColor(
                                data[i].lineColor ||
                                    'rgb(' + colors[i].join() + ')',
            ...
            setOnChange(id, varname, obj) {
                if (this.onchanges[varname]) {
                    this.onchanges[varname].obj.unbindChange(
                        this.onchanges[varname].id
                    );
                }
            ...
            makeSerie(data, value, name) {
                ...
                this.onActionReceive.removeSerieByName.call(
                    this,
                    data.name || {}
                );
                serie.setData(data.data);
        ...
        function analyzeContinuous(data) {
            if (Array.isArray(data)) {
                ...
                if (typeof data[0] === 'number') {
                    if (data.length < MIN_FOR_CONTINUOUS * 2 - 1)
                        return 'discrete';
