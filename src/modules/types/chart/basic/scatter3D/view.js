...
define([
  ...
  $.extend(true, View.prototype, Default, {
    _initThreejs: function () {
      ...
      var drawTickLabelsThrottled = _.throttle(
        this._drawTickLabels.bind(this),
        500,
      );
      var drawAxisLabelsThrottled = _.throttle(
        this._drawAxisLabels.bind(this),
        500,
      );
      var drawGraphTitleThrottled = _.throttle(
        this._drawGraphTitle.bind(this),
        500,
      );
      var projections = [];
      ...
      function showProjection() {
        ...
        var p1 = new THREE.Vector3(
          that._data.normalizedData.x[index],
      ...
      function init() {
        ...
        if (that.controls) {
          ...
          that.controls = new THREE.TrackballControls(
            that.camera,
            that.dom.get(0),
          );
        ...
        $(that.dom).append(
          '<div id="legend" style="z-index: 10000; right:10px ;position:absolute; top: 25px; height: auto; background-color: #ffffff;"> </div>',
        );
        var $legend = $('#legend');
        ...
        $legend
          .css(
            'background-color',
            that.module.getConfiguration('backgroundColor'),
          )
        ...
        $(that.renderer.domElement).on(
          'mousemove',
          _.throttle(onMouseMove, 100),
        );
        $(that.renderer.domElement).on('mousemove', _.throttle(onHover, 300));
        if (that._configCheckBox('tooltip', 'show')) {
          $(that.renderer.domElement).on(
            'mousemove',
            _.debounce(showTooltip, 500),
          );
          $(that.renderer.domElement).on(
            'mousemove',
            _.throttle(hideTooltip, 500),
          );
        }
    ...
    _normalizeData: function () {
      ...
      that._data.normalizedData.x = _.map(that._data.x, function (x) {
        return (
          (NORM_CONSTANT * (x - that._data.realMin.x)) / that._data.realLen.x
        );
      });
      that._data.normalizedData.y = _.map(that._data.y, function (y) {
        return (
          (NORM_CONSTANT * (y - that._data.realMin.y)) / that._data.realLen.y
        );
      });
      that._data.normalizedData.z = _.map(that._data.z, function (z) {
        return (
          (NORM_CONSTANT * (z - that._data.realMin.z)) / that._data.realLen.z
        );
      });
      ...
      that._data.size = _.map(that._data.size, function (s) {
        return sizeInt === 0
          ? sizeConstant / 2
          : sizeConstant * ((s - sizeMin) / sizeInt + 0.01);
      });
    },
    ...
    _processColors: function () {
      ...
      for (let i = 0; i < this._data.color.length; i++) {
    ...
    _getUnitPerTick: function (px, nbTick, valrange, axis) {
      ...
      var numberToNatural = unitPerTick * 10 ** -decimals;
      ...
      // Let's scale it back
      var unitPerTickCorrect = closest * 10 ** decimals;
      /*
             Example:
      ...
      that._data.realLen[axis] =
        that._data.realMax[axis] - that._data.realMin[axis];
      that._data.nbTicks[axis] = Math.round(
        (that._data.realMax[axis] - that._data.realMin[axis]) /
          that._data.intervalVal[axis] +
          1,
      );
      that._data.intervalPx[axis] =
        NORM_CONSTANT / (that._data.nbTicks[axis] - 1);
      ...
      if (Math.abs(intdec) <= 1) {
        ...
        that._data.intervalFactor[axis] = 10 ** intdec;
      }
      ...
      if (!_.isFinite(that._data.intervalPx[axis])) {
        ...
        that._data.decimals[axis] =
          that._data[axis][0] === 0
            ? 0
            : Math.floor(Math.log(Math.abs(num)) / Math.log(10));
        that._data.intervalFactor[axis] = 10 ** that._data.decimals[axis];
    ...
    _drawAxes: function () {
      ...
      var axX = new THREE.ArrowHelper(
        vX,
        new THREE.Vector3(0, 0, NORM_CONSTANT),
        NORM_CONSTANT,
        color,
        1,
        1,
      );
      var axY = new THREE.ArrowHelper(
        vY,
        new THREE.Vector3(0, 0, NORM_CONSTANT),
        NORM_CONSTANT,
        color,
      ...
      var axZ = new THREE.ArrowHelper(
        vZ,
        new THREE.Vector3(NORM_CONSTANT, 0, NORM_CONSTANT),
        NORM_CONSTANT,
        color,
        1,
        1,
    ...
    _drawCircle: function (options) {
      ...
      if (options.rotationAxis) {
        m1.makeRotationAxis(
          new THREE.Vector3(
            options.rotationAxis.x || 0,
            options.rotationAxis.y || 0,
            options.rotationAxis.z || 0,
          ),
    ...
    _drawColorBar: function () {
      ...
      if (
        !this.module.getConfiguration('gradient') ||
        this.module.getConfiguration('gradient').length <= 1
      ) {
        return;
      }
    ...
    _setGridOrigin: function () {
      ...
      this.gorigin.x =
        (NORM_CONSTANT * (this.gorigin.x - this._data.realMin.x)) /
        this._data.realLen.x;
      this.gorigin.y =
        (NORM_CONSTANT * (this.gorigin.y - this._data.realMin.y)) /
        this._data.realLen.y;
      this.gorigin.z =
        (NORM_CONSTANT * (this.gorigin.z - this._data.realMin.z)) /
    ...
    _drawSecondaryGrid: function () {
      ...
      for (let i = 0; i < that._data.nbTicks.x - 1; i++) {
        for (let j = 1; j < jmax; j++) {
          if (this._configCheckBox('grid', 'xysec')) {
            that.secondaryGrid.push(
              that._drawLine(
                new THREE.Vector3(
                  that._data.intervalPx.x * i +
                    (that._data.intervalPx.x / jmax) * j,
                  0,
                  DELTA + that.gorigin.z,
                ),
                new THREE.Vector3(
                  that._data.intervalPx.x * i +
                    (that._data.intervalPx.x / jmax) * j,
      ...
      // y lines
      for (let i = 0; i < that._data.nbTicks.y - 1; i++) {
        for (let j = 1; j < jmax; j++) {
          if (this._configCheckBox('grid', 'yzsec')) {
            that.secondaryGrid.push(
              that._drawLine(
                new THREE.Vector3(
                  DELTA + that.gorigin.x,
                  that._data.intervalPx.y * i +
                    (that._data.intervalPx.y / jmax) * j,
                  0,
                ),
                new THREE.Vector3(
                  DELTA + that.gorigin.x,
      ...
      // z lines
      for (let i = 0; i < that._data.nbTicks.z - 1; i++) {
        for (let j = 1; j < jmax; j++) {
          if (this._configCheckBox('grid', 'yzsec')) {
            that.secondaryGrid.push(
              that._drawLine(
                new THREE.Vector3(
                  DELTA + that.gorigin.x,
                  0,
                  that._data.intervalPx.z * i +
                    (that._data.intervalPx.z / jmax) * j,
                ),
                new THREE.Vector3(
                  DELTA + that.gorigin.x,
                  NORM_CONSTANT,
    ...
    _drawGrid: function () {
      ...
      // x lines
      for (let i = 0; i < that._data.nbTicks.x; i++) {
      ...
      // y lines
      for (let i = 0; i < that._data.nbTicks.y; i++) {
      ...
      // z lines
      for (let i = 0; i < that._data.nbTicks.z; i++) {
        if (this._configCheckBox('grid', 'yz')) {
          that.grid.push(
            that._drawLine(
              new THREE.Vector3(
    ...
    _drawTicks: function () {
      ...
      if (that._configCheckBox('ticks', 'x')) {
        for (let i = 0; i < that._data.nbTicks.x; i++) {
          that.ticks.push(
            that._drawLine(
              new THREE.Vector3(that._data.intervalPx.x * i, 0, NORM_CONSTANT),
              new THREE.Vector3(
                that._data.intervalPx.x * i,
                0,
                NORM_CONSTANT * 1.05,
      ...
      if (that._configCheckBox('ticks', 'y')) {
        for (let i = 0; i < that._data.nbTicks.y; i++) {
          that.ticks.push(
            that._drawLine(
              new THREE.Vector3(0, that._data.intervalPx.y * i, NORM_CONSTANT),
              new THREE.Vector3(
                -0.05 * NORM_CONSTANT,
                that._data.intervalPx.y * i,
                NORM_CONSTANT,
      ...
      if (that._configCheckBox('ticks', 'z')) {
        for (let i = 0; i < that._data.nbTicks.z; i++) {
          that.ticks.push(
            that._drawLine(
              new THREE.Vector3(NORM_CONSTANT, 0, that._data.intervalPx.z * i),
              new THREE.Vector3(
                NORM_CONSTANT * 1.05,
                0,
                that._data.intervalPx.z * i,
    ...
    _addText: function (text, x, y, z, options) {
      ...
      options.fillStyle =
        options.fillStyle ||
        colorUtil.array2rgba(that.module.getConfiguration('annotationColor')) ||
        DEFAULT_TEXT_COLOR;
    ...
    _drawTickLabels: function () {
      ...
      if (that._configCheckBox('ticks', 'zlab')) {
        for (let i = 0; i < that._data.nbTicks.z; i++) {
          const text = keepDecimals(
            (that._data.realMin.z + i * that._data.intervalVal.z) /
              that._data.intervalFactor.z,
            2,
          ).toString();
      ...
      if (that._configCheckBox('ticks', 'ylab')) {
        for (let i = 0; i < that._data.nbTicks.y; i++) {
          const text = keepDecimals(
            (that._data.realMin.y + i * that._data.intervalVal.y) /
              that._data.intervalFactor.y,
            2,
          ).toString();
          that.tickLabels.push(
            that._addText(
      ...
      if (that._configCheckBox('ticks', 'xlab')) {
        for (let i = 0; i < that._data.nbTicks.x; i++) {
          const text = keepDecimals(
            (that._data.realMin.x + i * that._data.intervalVal.x) /
              that._data.intervalFactor.x,
            2,
          ).toString();
          that.tickLabels.push(
            that._addText(
              text,
              i * that._data.intervalPx.x,
              0,
              NORM_CONSTANT * 1.1,
              {
                textAlign: 'right',
    ...
    _drawGraphTitle: function () {
      ...
      switch (mode) {
        case 'none':
          return;
        default:
          that.graphTitle.push(
            that._addText(title, NORM_CONSTANT / 10, NORM_CONSTANT * 1.3, 100, {
              textAlign: 'left',
            }),
          );
          break;
    ...
    _drawAxisLabels: function () {
      ...
      var mode = that.module.getConfiguration('labels');
      var xt = that._meta.getChildSync([
        'axis',
        that._data.xAxis || 0,
        'label',
      ]);
      var yt = that._meta.getChildSync([
        'axis',
        that._data.yAxis || 1,
        'label',
      ]);
      var zt = that._meta.getChildSync([
        'axis',
        that._data.zAxis || 2,
        'label',
      ]);
    ...
    _drawFaces: function () {
      ...
      var geometry1 = new THREE.PlaneBufferGeometry(
        NORM_CONSTANT,
        NORM_CONSTANT,
      );
      var geometry2 = new THREE.PlaneBufferGeometry(
        NORM_CONSTANT,
        NORM_CONSTANT,
      );
      var geometry3 = new THREE.PlaneBufferGeometry(
        NORM_CONSTANT,
        NORM_CONSTANT,
      );
      var geometry4 = new THREE.PlaneBufferGeometry(
        NORM_CONSTANT,
        NORM_CONSTANT,
      );
      var geometry5 = new THREE.PlaneBufferGeometry(
        NORM_CONSTANT,
        NORM_CONSTANT,
      );
      var geometry6 = new THREE.PlaneBufferGeometry(
        NORM_CONSTANT,
        NORM_CONSTANT,
      );
    ...
    _mathPoints: function () {
      ...
      for (let i = 0; i < that._data.x.length; i++) {
        ...
        var sphere = new THREE.Sphere(
          new THREE.Vector3(
            that._data.normalizedData.x[i],
            that._data.normalizedData.y[i],
            that._data.normalizedData.z[i],
          ),
          radius * NORM_CONSTANT,
        );
        sphere.index = i;
    ...
    _newParticleObject: function (indexes, options) {
      ...
      options = options || {};
      var image =
        shapeImages[options.shape] || shapeImages[DEFAULT_POINT_SHAPE];
      ...
      if (indexes) {
        ...
        for (let i = 0; i < that._data.normalizedData.x.length; i++) {
          const vertex = new THREE.Vector3();
        ...
        for (let v = 0; v < vertices.length; v++) {
          ...
          if (forcedColor) {
            ...
            values_color[v] = new THREE.Color(
              color[indexes[v]] || DEFAULT_POINT_COLOR,
            );
          }
        }
      } else {
    ...
    _prepareHighlights: function (hl) {
      ...
      for (var shape in m) {
        for (var hlkey in m[shape]) {
          that._highlightParticleObjects[shape] =
            that._highlightParticleObjects[shape] || {};
          that._highlightParticleObjects[shape][hlkey] =
            that._newParticleObject(m[shape][hlkey], {
    ...
    update: {
      ...
      boolArray: function (moduleValue) {
        ...
        for (const shape in that._highlightParticleObjects) {
          for (const hlkey in that._highlightParticleObjects[shape]) {
            that._updateParticleObject(
              that._highlightParticleObjects[shape][hlkey],
              {
                applyFilter: true,
                updateColor: false,
    ...
    _convertData3dToData: function (value) {
      ...
      for (let i = 0; i < value.length; i++) {
        ...
        that._data.color.push(
          getFromJpath(value, jp.color, DEFAULT_POINT_COLOR),
        );
        that._data.size.push(
          getFromJpath(value, jp.size, DEFAULT_POINT_RADIUS),
        );
        that._data.shape.push(
          getFromJpath(value, jp.shape, DEFAULT_POINT_SHAPE),
        );
      }
    ...
    _completeData: function (name, defaultValue) {
      ...
      for (let i = 0; i < that._data.x.length; i++) {
        if (that._data[name][i] === undefined) {
          that._data[name][i] = defaultValue;
        }
      }
    },
    ...
    _activateHighlights: function () {
      ...
      function undrawHighlightBis(hl) {
        ...
        for (var shape in that._highlightParticleObjects) {
          if (
            that._highlightParticleObjects[shape][hl] &&
            that._highlightParticleObjects[shape][hl].drawn
          ) {
