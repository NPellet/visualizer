...
define(
    ...
    function (Default, Util, _, API, Renderer) {
                        ...
                        fields: {
                            slickCheck: {
                                ...
                                options: {
                                    ...
                                    rowNumbering:
                                        'Show row number when scrolling',
                                    oneUncollapsed:
                                        'Maximum One group uncollapsed (per level)',
                                    filterColumns: 'Enable search header',
                                    keepSelected:
                                        'Keep selected elements when filtering',
                                    backToTop: "Don't remember scroll position",
                                    forgetLastSelected:
                                        "Don't remember last selected rows",
                                    forgetLastActive:
                                        "Don't remember last active cell",
                                    highlightScroll:
                                        'Scroll to highlighted line',
                                    collapseGroup: 'Collapse groups on start',
                                    ignoreMyHighlights:
                                        'Ignore highlights from myself'
                                },
                                default: [
                            ...
                            copyPasteOptions: {
                                ...
                                options: {
                                    readOnly: 'Read only',
                                    newRows:
                                        'Paste to new rows when overflowing',
                                    noAutoFocus:
                                        'Disable auto focus when row selection changes'
                                },
                                displayTarget: ['cp'],
                                default: ['newRows']
                            },
        ...
        Controller.prototype.onBeforeSave = function (formValue) {
            var varname =
                formValue.module_specific_config[0].groups.data[0].varname[0];
            var saveInView =
                formValue.module_specific_config[0].groups.data[0].saveInView[0]
                    .length;
            ...
            if (varname && saveInView) {
                ...
                formValue.vars_in[0].groups.group[0] = vars_in.filter(function (
                    v
                ) {
