...
if (s.contains ("is2D:true")) this.set2D ();
if (s.contains ("scale:")) this.getScaling (this.getSection (s, "scale", false));
s = JU.PT.replaceAllCharacters (s, "}", "").$replace (',', ':');
...
break;
case 'o':
o = this.parseIntStr (beo[j + 1]);
break;
}

this.atomSetCollection.addBond ( new J.adapter.smarter.Bond (b, e, o));
}
}, $fz.isPrivate = true, $fz), "~A");
});
