...
Graph.prototype = {
	...
	handleMouseMove: function(x, y, e) {
		...
		x = x - this.options.paddingLeft;

		for(var i = 0; i < this.xaxis.length; i++)
			this.xaxis[i].handleMouseMove(x, e);
	
		for(var i = 0; i < this.leftyaxis.length; i++)
			this.leftyaxis[i].handleMouseMove(y, e);
		
		for(var i = 0; i < this.rightyaxis.length; i++)
			this.rightyaxis[i].handleMouseMove(y, e);	
		
		var results = {};
		for(var i = 0; i < this.series.length; i++)
			results[this.series[i].getName()] = this.series[i].handleMouseMove(x, y, e);
	...
	getXAxis: function(num) {
		num = num || 0;
		return this.xaxis[num] = this.xaxis[num] || new GraphXAxis(this);
	},

	getLeftAxis: function(num) {
		num = num || 0;
		return this.leftyaxis[num] = this.leftyaxis[num] || new GraphYAxis(this, 'left');
	},

	getRightAxis: function(num) {
		num = num || 0;
		return this.rightyaxis[num] = this.rightyaxis[num] || new GraphYAxis(this, 'right');
	},
...
console.log(this.width, this.height);
	...
	refreshDrawingZone: function() {
		...
		for(i = this.leftyaxis.length - 1; i >= 0; i--) {
			...
			this.leftyaxis[i].setMaxPx(this.getDrawingHeight(true) - shift[0] + this.getPaddingTop());
		...
		if(this.options.closeRight && this.rightyaxis.length == 0) {
			...
			this.rightLine.setAttribute('y2', this.getDrawingHeight(true) + this.getPaddingTop() - shift[0]);
		} else {
			this.rightLine.setAttribute('display', 'none');
		}
		...
		this.clipRect.setAttribute('height', this.getDrawingHeight() - shift[0] + this.options.paddingTop);
...
GraphAxis.prototype = {
	...
	setAxisDataSpacing: function(val1, val2) {
		this.axisDataSpacing.min = val1;
		this.axisDataSpacing.max = val2 || val1;
	},

	setAxisDataSpacingMin: function(val) {
		this.axisDataSpacing.min = val;
	},

	setAxisDataSpacingMax: function(val) {
		this.axisDataSpacing.max = val;
	},
	...
	getNbTicksSecondary: function() {
		return this.nbTicksSecondary || 5;
	},
	...
	_recalculateDataInterval: function() {
		...
		this._realMin = this.getMin() - (this.axisDataSpacing.min * interval);
		this._realMax = this.getMax() + (this.axisDataSpacing.max * interval);
	},
	...
	draw: function() {
		...
		if(this.lineAt0 && this.getActualMin() < 0 && this.getActualMax() > 0)
	...
	getExponentialFactor: function() {
		return (this.exponentialFactor = this.exponentialFactor || 0);
	},

	setExponentialFactor: function(value) {
		this.exponentialFactor = value;
	},

	setExponentialLabelFactor: function(value) {
		this.exponentialLabelFactor = value;
	},

	getExponentialLabelFactor: function() {
		return (this.exponentialLabelFactor = this.exponentialLabelFactor || 0);
	},
	...
	togglePrimaryGrid: function(bool) {
		this.primaryGrid = bool;
	},

	toggleSecondaryGrid: function(bool) {
		this.secondaryGrid = bool;
	},
...
$.extend(GraphXAxis.prototype, GraphAxis.prototype, {
	...
	_setShift: function() {
		...
		this.group.setAttribute('transform', 'translate(0 ' + (this.graph.getDrawingHeight() + this.graph.getPaddingTop() - this.shift) + ')')
	},
	...
	drawTick: function(value, label, scaling) {
		...
		if(label && this.primaryGrid)
			this.doGridLine(true, val, val, 0, this.getMaxPx());
		else if(!label && this.secondaryGrid)
		...
		if(label) {
			...
			tickLabel.setAttribute('y', (this.tickPosition == 1) ? 15 : 25);
...
GraphYAxis = function(graph, leftright) {
	this.init(graph);
...
GraphSerie.prototype = {
	...
	draw: function() {
		...
		if(!this.graph.painted && !this.graph.redraw())
			return;
		...
		for(; i < l ; i++) {
			...
			for(; j < this.data[i].length; j+=2) {
				...
				xpx = this.getXAxis().getPx(this.data[i][j]);
				ypx = this.getYAxis().getPx(this.data[i][j + 1]);
