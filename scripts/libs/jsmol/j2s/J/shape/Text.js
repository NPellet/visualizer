...
this.z -= Clazz.floatToInt (this.pymolOffset[3] * pixelsPerAngstrom);
pixelsPerAngstrom = viewer.scaleToScreen (this.z, 1000);
var isOld = (this.pymolOffset[1] >= -1 && this.pymolOffset[1] <= 1 && this.pymolOffset[2] >= -1 && this.pymolOffset[2] <= 1);
if (isOld) {
dx = this.textWidth * (this.pymolOffset[1] - 1) / 2;
dy = -this.textHeight * ((this.pymolOffset[2] - 1) / 2);
dy += this.descent;
} else {
dx = this.pymolOffset[1] * pixelsPerAngstrom;
dy = -this.pymolOffset[2] * pixelsPerAngstrom;
dy *= 1.05;
